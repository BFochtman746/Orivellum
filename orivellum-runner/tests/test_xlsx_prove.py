...
pytestmark = pytest.mark.skipif(not engine.available(), reason="formulas engine not installed")
...
def test_reorder_sheet_xml_produces_canonical_order():
    ...
    seq = [t for t in re.findall(r"<(\w+)[ >/]", fixed) if t in surgery.SHEET_ORDER]
...
def test_reorder_refuses_unknown_top_level_content():
    ...
    assert not surgery.sheet_order_violations(fixed)
    unknown = BAD_SHEET.replace("</worksheet>", "<mysteryTag><a/></mysteryTag></worksheet>")
...
def test_prove_fails_and_ships_nothing_on_true_errors(tmp_path):
    ...
    ws = wb.active
    ws["A1"] = 1
    ws["B1"] = "=A1/0"  # genuinely errors on recalculation
    p = tmp_path / "bad.xlsx"
    wb.save(p)
    run_id = _make_run(target=p)
    digest = xlsx.prove_unit(run_id, {"target": str(p)})
...
def test_no_proven_file_left_when_gates_crash(tmp_path, monkeypatch):
    ...
    run_id = _make_run(target=p)
    monkeypatch.setattr(
        surgery, "parts_diff", lambda a, b: (_ for _ in ()).throw(RuntimeError("boom"))
    )
    with pytest.raises(RuntimeError):
        xlsx.prove_unit(run_id, {"target": str(p)})
    from runner.config import CFG

    leftovers = list(Path(CFG.runs_dir).glob("**/PROVEN_*")) + list(
        Path(CFG.runs_dir).glob("**/.candidate_*")
    )
