...
def start_run(job, target, label, plan):
    ...
    with conn() as c:
        return c.execute(
            "INSERT INTO runs(started,job,target,label,plan) VALUES(?,?,?,?,?)",
            (now(), job, target, label, json.dumps(plan)),
        ).lastrowid
...
def add_units(run_id, units):
    with conn() as c:
        c.executemany(
            "INSERT INTO units(run_id,ord,kind,ref,payload) VALUES(?,?,?,?,?)",
            [
...
def next_unit(run_id):
    ...
    with conn() as c:
        while True:
            r = c.execute(
                "SELECT * FROM units WHERE run_id=? AND status='queued' ORDER BY ord LIMIT 1",
                (run_id,),
            ).fetchone()
            if not r:
                return None
            claimed = c.execute(
                "UPDATE units SET status='running' WHERE id=? AND status='queued'", (r["id"],)
            ).rowcount
            if claimed:
                d = dict(r)
...
def requeue_running(run_id):
    ...
    with conn() as c:
        return c.execute(
            "UPDATE units SET status='queued' WHERE run_id=? AND status='running'", (run_id,)
        ).rowcount
...
def digests(run_id, kind=None, limit=100000):
    q = "SELECT * FROM units WHERE run_id=? AND status='done'"
    a = [run_id]
    if kind:
        q += " AND kind=?"
        a.append(kind)
    ...
    with conn() as c:
        ...
        for r in c.execute(q, a):
            d = dict(r)
            d["digest"] = json.loads(d["digest"] or "{}")
            d["payload"] = json.loads(d["payload"] or "{}")
            out.append(d)
        return out
...
def add_finding(
    run_id, severity, code, ref, title, detail="", fix="", source="analysis", unique=False
):
    with conn() as c:
        if unique:
            ...
            c.execute(
                "DELETE FROM findings WHERE run_id=? AND code=? AND ref=?", (run_id, code, ref)
            )
        c.execute(
            """INSERT INTO findings(run_id,severity,code,ref,title,detail,fix,source)
                     VALUES(?,?,?,?,?,?,?,?)""",
...
def get_notes(run_id):
    with conn() as c:
        return [
            dict(r)
            for r in c.execute("SELECT at,text FROM notes WHERE run_id=? ORDER BY id", (run_id,))
        ]
...
def end_run(run_id, status, reason, totals):
    with conn() as c:
        c.execute(
            "UPDATE runs SET finished=?, status=?, stop_reason=?, totals=? WHERE id=?",
