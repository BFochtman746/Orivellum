...
from . import store, harness, report, llm  # noqa: F401 (json/llm: kept for job modules & debugging)
from .jobs import code as code_job, xlsx as xlsx_job

JOBS = {"code": code_job, "xlsx": xlsx_job}

def _run_dir(run_id):
    d = Path(CFG.runs_dir) / str(run_id); d.mkdir(parents=True, exist_ok=True); return d
...
def cmd_list(_a):
    ...
    if not rows: print("no runs yet"); return 0
    print(f"{'ID':<5}{'JOB':<7}{'STATUS':<10}{'STARTED':<21}TARGET")
    for r in rows:
        print(f"{r['id']:<5}{r['job']:<7}{r['status']:<10}{r['started']:<21}"
              f"{Path(r['target']).name}")
    return 0
...
def cmd_verify(a):
    ...
    if res.get("error"): print(f"  engine: {res['error']}")
    for f in res.get("failed", [])[:20]:
        print(f"  FAIL {f['sheet']}!{f['cell']}: expected {f['expected']}, "
...
def cmd_findings(a):
    ...
    if a.severity: f = [x for x in f if x["severity"] == a.severity.upper()]
    for x in f:
        print(f"[{x['severity']:<8}] {x['code']:<20} {x['ref'] or ''}")
        print(f"           {x['title']}")
        if x["fix"]: print(f"           FIX: {x['fix']}")
    print(f"\n{len(f)} finding(s)")
    return 0
...
def main(argv=None):
    ...
    r = s.add_parser("run"); r.add_argument("--job", choices=list(JOBS), required=True)
    r.add_argument("--target", required=True); r.add_argument("--label", default="baseline")
    r.set_defaults(fn=cmd_run)
    rs = s.add_parser("resume"); rs.add_argument("--run", type=int, required=True); rs.set_defaults(fn=cmd_resume)
    rp = s.add_parser("report"); rp.add_argument("--run", type=int, required=True); rp.set_defaults(fn=cmd_report)
    s.add_parser("list").set_defaults(fn=cmd_list)
    fd = s.add_parser("findings"); fd.add_argument("--run", type=int, required=True)
    fd.add_argument("--severity"); fd.set_defaults(fn=cmd_findings)
    vf = s.add_parser("verify"); vf.add_argument("--target", required=True)
    vf.add_argument("--tests", required=True); vf.set_defaults(fn=cmd_verify)
