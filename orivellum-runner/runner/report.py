...
def render(run_id):
    ...
    L.append(f"**Started** {run['started']}  ·  **Finished** {run['finished'] or '—'}  ·  "
             f"**Status** `{run['status']}`\n")
    L.append(f"**Why it stopped:** {run['stop_reason']}\n")

    L.append("\n## Completeness — read this before the findings\n")
    L.append(f"- Units processed: **{counts.get('done',0)}** of "
             f"{sum(counts.values())}\n")
    if counts.get("failed"): L.append(f"- Units FAILED: **{counts['failed']}**\n")
    if run["totals"].get("remaining"):
        L.append(f"- Units never reached: **{run['totals']['remaining']}** "
                 f"— this report is PARTIAL\n")
    L.append(f"- Elapsed: {run['totals'].get('elapsed_min','?')} min  ·  "
             f"model calls: {run['totals'].get('llm',{}).get('calls',0)}  ·  "
             f"est. tokens: {run['totals'].get('llm',{}).get('est_tokens',0):,}\n")
    plan = run.get("plan") or {}
    if plan.get("unavailable"):
        L.append(f"- Tools unavailable (reported as unknown, **not** as clean): "
                 f"{', '.join(plan['unavailable'])}\n")

    L.append("\n## Findings\n")
    if not f:
        L.append("Nothing was flagged. With scanners unavailable that means "
                 "*unexamined*, not *clean*.\n")
    else:
        L.append("| Severity | Code | Where | Finding |\n| --- | --- | --- | --- |\n")
        for x in f[:400]:
            L.append(f"| {x['severity']} | `{x['code']}` | `{(x['ref'] or '')[:52]}` | "
                     f"{x['title']} |\n")
        if len(f) > 400: L.append(f"\n_{len(f)-400} further findings in the database._\n")
        L.append("\n**By severity:** " + " · ".join(
            f"{s} {sev[s]}" for s in SEV_ORDER if sev[s]) + "\n")
    ...
    if fails:
        ...
        for u in fails[:60]:
            L.append(f"| {u['ord']} | `{u['ref'][:52]}` | {u['attempts']} | {(u['err'] or '')[:90]} |\n")

    notes = store.get_notes(run_id)
    if notes:
        L.append("\n## Run log\n\n```\n")
        for n in notes[-60:]: L.append(f"{n['at']}  {n['text']}\n")
        L.append("```\n")
    return "".join(L)
...
def training_plan(run_id, items):
    ...
    L.append("\n---\n\nGaps this plan does not close: judgement calls about "
             ...
             "reading list.\n")
    d = Path(CFG.runs_dir) / str(run_id); d.mkdir(parents=True, exist_ok=True)
    p = d / "TRAINING_PLAN.md"; p.write_text("".join(L), encoding="utf-8")
    return str(p)
