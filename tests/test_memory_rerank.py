...
def _fact(mid: str, key: str = "k", value: str = "v", rrf: float = 0.01) -> dict:
    return {
        ...
        "valid_to": None,
        "txn_time": "2026-01-01T00:00:00+00:00",
...
class TestGraphBoostScores(unittest.TestCase):
    ...
    def test_entity_mention_boosts_rrf_score(self):
        ...
        self.assertGreater(
            result[0]["rrf_score"], 0.01, "Entity match must boost rrf_score above the original"
        )
...
class TestCrossEncoderRerank(unittest.TestCase):
    ...
    def test_candidates_beyond_limit_appended_unchanged(self):
        ...
        from orivellum.capabilities.llm import LLMResult
        from orivellum.capabilities.memory import _CE_MAX_CANDIDATES, _cross_encoder_rerank
        ...
        with patch(
            "orivellum.capabilities.llm.llm_call", return_value=LLMResult("8", True, "model", 10)
        ):
...
class TestQueryComplexityScore(unittest.TestCase):
    ...
    def test_long_query_adds_score(self):
        ...
        long_q = (
            "can you summarise everything I have said about machine learning frameworks and tooling"
        )
    ...
    def test_highly_complex_query_exceeds_threshold(self):
        from orivellum.capabilities.memory import _COMPLEXITY_THRESHOLD, query_complexity_score
...
class TestReActMemoryAgent(unittest.TestCase):
    ...
    def test_done_flag_halts_loop_after_first_iteration(self):
        ...
        with patch(
            ...
            return_value=self._llm_result('{"tool":"lexical_search","query":"Python","done":true}'),
        ):
    ...
    def test_lexical_tool_finds_seeded_fact(self):
        ...
        agent = ReActMemoryAgent(self.db, self._cfg())
        with patch(
            "orivellum.capabilities.llm.llm_call", side_effect=lambda *a, **kw: next(responses)
        ):
    ...
    def test_unknown_tool_falls_back_to_lexical(self):
        ...
        with patch(
            "orivellum.capabilities.llm.llm_call",
            return_value=self._llm_result('{"tool":"mind_read","query":"anything","done":false}'),
        ):
    ...
    def test_deduplication_by_id(self):
        ...
        self.assertEqual(
            ids.count("shared-id"), 1, "Same fact from multiple iterations must be deduplicated"
        )
    ...
    def test_max_iter_cap(self):
        ...
        agent = ReActMemoryAgent(self.db, self._cfg())
        with (
            patch("orivellum.capabilities.llm.llm_call", side_effect=_llm),
            patch("orivellum.capabilities.memory._channel_lexical", side_effect=_new_facts),
        ):
            agent.run("test query")

        self.assertLessEqual(
            call_count[0], ReActMemoryAgent.MAX_ITER, "LLM must be called at most MAX_ITER times"
        )
...
class TestRerankMemories(unittest.TestCase):
    ...
    def _cands(self, n: int = 5) -> list[dict]:
        return [
            _fact(f"id-{i}", f"key{i}", f"value number {i} contains text", rrf=0.1)
            for i in range(n)
        ]
    ...
    def test_cross_encoder_runs_when_enabled(self):
        ...
        with (
            patch("orivellum.capabilities.llm.llm_call", side_effect=_fake_llm),
    ...
    def test_stage_failure_does_not_raise(self):
        ...
        with patch(
            "orivellum.capabilities.memory._graph_boost_scores",
...
class TestSearchAndRerankMemories(unittest.TestCase):
    ...
    def test_simple_query_does_not_use_react(self):
        ...
        self.assertFalse(
            meta["react_used"], "Simple single-token query must not trigger ReAct agent"
        )
    ...
    def test_complex_query_triggers_react(self):
        ...
        self.assertTrue(
            meta["react_used"], "Complex query must trigger ReAct agent (react_used=True)"
        )
    ...
    def test_react_failure_falls_back_to_hybrid(self):
        ...
        with (
            patch("orivellum.capabilities.memory.ReActMemoryAgent") as MockAgent,
            patch("orivellum.capabilities.memory._channel_semantic", return_value=[]),
            patch("orivellum.configuration.config.load_config") as _lc,
        ):
            cfg = MagicMock()
...
class TestReActEmptyResultFallback(unittest.TestCase):
    ...
    def test_react_done_immediately_still_returns_hybrid_facts(self):
        ...
        self.assertIn(
            "python_pref", keys, "The seeded python_pref fact must be surfaced via hybrid fallback"
        )
    ...
    def test_react_all_tools_return_empty_falls_back_to_hybrid(self):
        ...
        self.assertGreater(
            len(ranked), 0, "ReAct returning no hits must fall back to hybrid, not return []"
        )
    ...
    def test_react_malformed_json_falls_back_to_hybrid(self):
        ...
        self.assertGreater(
            len(ranked), 0, "Malformed LLM output must fall back to hybrid, not return []"
        )
    ...
    def test_react_llm_unavailable_falls_back_to_hybrid(self):
        ...
        self.assertGreater(
            len(ranked), 0, "LLM unavailable must fall back to hybrid, not return []"
        )
...
class TestListwiseRRFFusion(unittest.TestCase):
    ...
    def test_listwise_consensus_ranks_above_single_channel(self):
        ...
        with (
            patch("orivellum.capabilities.rerank._llm_rerank", return_value=[0, 1, 2]),
            patch("orivellum.configuration.config.load_config") as _lc,
            patch(
                "orivellum.capabilities.llm.llm_call",
    ...
    def test_listwise_stage_ran_field_true_when_llm_enabled(self):
        ...
        with (
            patch("orivellum.capabilities.rerank._llm_rerank", return_value=[0, 1, 2, 3, 4]),
            patch("orivellum.configuration.config.load_config") as _lc,
            patch(
                "orivellum.capabilities.llm.llm_call",
                return_value=LLMResult("8", True, "model", 10),
    ...
    def test_listwise_does_not_fire_with_fewer_than_3_candidates(self):
        ...
        lw_stage = next(s for s in meta["stages"] if s["name"] == "listwise")
        self.assertFalse(
            lw_stage.get("ran"), "Listwise must not run when fewer than 3 candidates remain"
        )
    ...
    def test_listwise_uses_bm25_rank_not_cross_encoder_rank(self):
        ...
        with (
            patch("orivellum.capabilities.rerank.bm25_rerank", side_effect=_mock_bm25),
            patch("orivellum.capabilities.rerank._llm_rerank", return_value=[2, 1, 0]),
...
class TestGetMemoryRetrievalStages(unittest.TestCase):
    ...
    def test_retrieval_stages_present_in_response(self):
        ...
        with (
            _patch("orivellum.api.routes.conversations.get_db", return_value=self.db),
            _patch("orivellum.capabilities.memory._channel_semantic", return_value=[]),
        ):
            result = asyncio.run(get_memory(q="zsh shell", include_evidence=False))

        self.assertIn(
            "retrieval_stages", result, "Response must include 'retrieval_stages' when ?q= is set"
        )
    ...
    def test_without_q_no_retrieval_stages(self):
        ...
        with _patch("orivellum.api.routes.conversations.get_db", return_value=self.db):
            result = asyncio.run(get_memory(q=None, include_evidence=False))

        self.assertNotIn(
            "retrieval_stages", result, "Non-query path must not include retrieval_stages"
        )
