...
class TestSchemaV99(unittest.TestCase):
    ...
    def test_memory_evidence_table_exists(self):
        ...
        self.assertIn(
            "memory_evidence", tables, "memory_evidence table must exist after v99 migration"
        )

    def test_memory_evidence_columns(self):
        cols = {
            r[1] for r in self.db._conn.execute("PRAGMA table_info(memory_evidence)").fetchall()
        }
        for col in (
            "id",
            "raw_text",
    ...
    def test_user_memory_has_source_evidence_id(self):
        ...
        self.assertIn(
            "source_evidence_id", cols, "user_memory must have source_evidence_id after v99"
        )
...
class TestCreateMemoryEvidence(unittest.TestCase):
    ...
    def test_row_is_retrievable(self):
        ...
        eid = self.db.create_memory_evidence(
            raw, source_type="conversation", source_id="conv-1", conversation_id="conv-1"
        )
    ...
    def test_raw_text_truncated_to_2000_chars(self):
        ...
        self.assertLessEqual(
            len(row["raw_text"]), 2000, "raw_text must be truncated to at most 2000 characters"
        )
...
class TestUpsertWithEvidence(unittest.TestCase):
    ...
    def test_source_evidence_id_stored_in_row(self):
        ...
        self.db.upsert_memory_fact("lang", "Python", memory_type="semantic", source_evidence_id=eid)
    ...
    def test_fact_without_evidence_has_null_source_evidence_id(self):
        ...
        self.assertIsNotNone(row)
        self.assertIsNone(
            row["source_evidence_id"], "source_evidence_id must be NULL when no evidence passed"
        )
...
class TestGetCurrentWithEvidence(unittest.TestCase):
    ...
    def test_fact_without_evidence_has_null_evidence_fields(self):
        ...
        self.assertIsNotNone(fact)
        self.assertIsNone(
            fact.get("evidence_text"), "evidence_text must be NULL for facts without evidence"
        )
    ...
    def test_include_evidence_false_omits_evidence_fields(self):
        ...
        self.assertIsNotNone(fact)
        self.assertNotIn(
            "evidence_text", fact, "evidence_text must not appear when include_evidence=False"
        )
...
class TestEvidenceBeliefOrdering(unittest.TestCase):
    ...
    def _run_infer(
        self, user_text: str, assistant_text: str, llm_response: str = '{"facts": []}'
    ) -> list[dict]:
        ...
        def capture_evidence_then_return(*args, **kwargs):
            ...
            rows = self.db._conn.execute("SELECT * FROM memory_evidence").fetchall()
        ...
        with (
            patch.object(conv_mod, "get_config", return_value=mock_cfg),
            patch(
    ...
    def test_evidence_committed_before_llm_invoked(self):
        ...
        evidence_rows = self._run_infer(
            "I really prefer Python over Go for backend work.", "Understood, I'll remember that."
        )
        self.assertGreater(
            len(evidence_rows), 0, "Evidence row must be committed before _call_sync is called"
        )

    def test_evidence_text_matches_exchange(self):
        """The committed evidence must contain the user text from the exchange."""
        evidence_rows = self._run_infer("My favourite language is Rust.", "Got it, noted.")
        self.assertTrue(
    ...
    def test_evidence_conversation_id_set_before_llm(self):
        ...
        evidence_rows = self._run_infer("I work mostly on distributed systems.", "Good to know.")
        self.assertTrue(
    ...
    def test_short_exchange_writes_no_evidence(self):
        ...
        count = self.db._conn.execute("SELECT COUNT(*) AS n FROM memory_evidence").fetchone()["n"]
    ...
    def test_evidence_write_failure_prevents_fact_creation(self):
        ...
        with (
            patch.object(conv_mod, "get_config", return_value=mock_cfg),
            patch("orivellum.capabilities.cognition._call_sync", return_value=llm_response),
            patch.object(self.db, "create_memory_evidence", side_effect=RuntimeError("DB locked")),
        ):
            # Must not raise
            _infer_memory_facts(
                self.db, "conv-test", "I prefer Python for all my work.", "Understood."
            )

        # No fact should have been written because evidence write failed
        facts = self.db.get_current_memory_facts()
        self.assertEqual(len(facts), 0, "No fact must be written when evidence creation fails")
...
class TestNoFactEvidenceCleanup(unittest.TestCase):
    ...
    def test_no_facts_deletes_evidence_row(self):
        ...
        self._run_infer("I think the weather is nice today.", '{"facts": []}')
        count = self.db._conn.execute("SELECT COUNT(*) AS n FROM memory_evidence").fetchone()["n"]
        self.assertEqual(count, 0, "Evidence row must be deleted when no facts are derived")
    ...
    def test_below_threshold_facts_deletes_evidence(self):
        ...
        count = self.db._conn.execute("SELECT COUNT(*) AS n FROM memory_evidence").fetchone()["n"]
        self.assertEqual(count, 0, "Evidence must be deleted when all facts are below threshold")
    ...
    def test_qualifying_fact_retains_evidence(self):
        ...
        count = self.db._conn.execute("SELECT COUNT(*) AS n FROM memory_evidence").fetchone()["n"]
        self.assertEqual(
            count, 1, "Evidence row must be retained when at least one fact is written"
        )
    ...
    def test_llm_empty_response_deletes_evidence(self):
        ...
        with (
            ...
            _infer_memory_facts(
                self.db, "conv-test", "I occasionally use dark mode sometimes.", "Got it."
            )

        count = self.db._conn.execute("SELECT COUNT(*) AS n FROM memory_evidence").fetchone()["n"]
        self.assertEqual(count, 0, "Evidence must be deleted when LLM returns empty response")
...
class TestHandleRememberEvidenceOrdering(unittest.TestCase):
    ...
    def _run_remember(
        self, user_text: str, llm_response: str = '{"key": "lang", "value": "Python"}'
    ):
        import sys
        ...
        with patch("orivellum.capabilities.cognition._call_sync", side_effect=capture_then_return):
    ...
    def test_evidence_write_failure_aborts_remember_capture(self):
        ...
        with (
            ...
            patch(
                "orivellum.capabilities.cognition._call_sync",
                return_value='{"key": "lang", "value": "Python"}',
            ) as mock_llm,
        ):
            reply = _handle_remember(
                self.db, "Remember I like Python.", "http://fake", "fake-model"
            )
    ...
    def test_successful_remember_reply_contains_key(self):
        reply, _ = self._run_remember("Remember I prefer Rust.", '{"key": "lang", "value": "Rust"}')
