...
class TestChapterKnowledge(unittest.TestCase):
    ...
    def test_items_ordered_by_kind_then_confidence(self):
        ...
        _seed_knowledge(
            self.db, self.work_id, self.chapter_id, "event", "Battle of the Bridge", confidence=0.9
        )
        _seed_knowledge(
            self.db, self.work_id, self.chapter_id, "character", "Alice (low)", confidence=0.5
        )
        _seed_knowledge(
            self.db, self.work_id, self.chapter_id, "character", "Bob (high)", confidence=0.95
        )
        _seed_knowledge(
            self.db, self.work_id, self.chapter_id, "setting", "Dark Forest", confidence=0.7
        )
        _seed_knowledge(
            self.db, self.work_id, self.chapter_id, "theme", "Redemption", confidence=0.6
        )
    ...
    def test_rejected_items_excluded(self):
        ...
        _seed_knowledge(
            self.db, self.work_id, self.chapter_id, "event", "AI event", review_status="ai_auto"
        )
