...
def _add_suggestion(db, kind: str, text: str, meta: dict,
...
class ReviewQueueTests(unittest.TestCase):
    ...
    def test_queue_aggregates_all_four_types(self):
        ...
        sid = _add_suggestion(self.db, "version_relationship", "A derives from B",
        ...
        self.assertEqual(body["counts_by_type"],
                         {"knowledge": 1, "reclassify": 1, "suggestion": 1, "duplicate": 1})

    def test_queue_sorted_confidence_asc(self):
        self.db.create_knowledge_item(None, "claim", "low", confidence=0.1,
                                      review_status="ai_auto")
        self.db.create_knowledge_item(None, "claim", "high", confidence=0.9,
                                      review_status="ai_auto")
    ...
    def test_resolve_knowledge_approve_and_reject(self):
        ...
        r = self.client.post(f"/api/review/knowledge:{k1}/resolve",
    ...
    def test_defer_snoozes_item_from_queue(self):
        kid = self.db.create_knowledge_item(None, "claim", "later",
                                            review_status="ai_auto")
        r = self.client.post(f"/api/review/knowledge:{kid}/resolve",
        ...
        with self.db._lock:
            self.db._conn.execute("UPDATE review_deferrals SET deferred_until=?",
    ...
    def test_resolve_reclassify_reject_deletes_row(self):
        ...
        rid = _add_reclassify(self.db, doc["id"])
        r = self.client.post(f"/api/review/reclassify:{rid}/resolve",
                             json={"decision": "reject"})
        self.assertEqual(r.status_code, 200)
        with self.db._lock:
            row = self.db._conn.execute(
                "SELECT 1 FROM pending_reclassify WHERE id=?", (rid,)).fetchone()
    ...
    def test_resolve_work_assignment_creates_work_and_links(self):
        ...
        sid = _add_suggestion(self.db, "work_assignment", "Group archive docs",
                              {"doc_ids": [d1["id"], d2["id"], d3["id"]],
                               "proposed_title": "My Archive"})
        r = self.client.post(f"/api/review/suggestion:{sid}/resolve",
                             json={"decision": "approve"})
        self.assertEqual(r.status_code, 200)
    ...
    def test_resolve_suggestion_reject_deletes(self):
        ...
        with self.db._lock:
            row = self.db._conn.execute(
                "SELECT 1 FROM suggestions WHERE id=?", (sid,)).fetchone()
        self.assertIsNone(row)
    ...
    def test_double_resolve_conflicts(self):
        ...
        d1, d2 = _make_doc(self.db), _make_doc(self.db)
        did = _add_dupe(self.db, d1["id"], d2["id"])
        self.client.post(f"/api/review/duplicate:{did}/resolve",
                         json={"decision": "reject"})
        r = self.client.post(f"/api/review/duplicate:{did}/resolve",
                             json={"decision": "approve"})
        self.assertEqual(r.status_code, 409)
        ...
        self.client.post(f"/api/review/suggestion:{sid}/resolve",
                         json={"decision": "approve"})
        r = self.client.post(f"/api/review/suggestion:{sid}/resolve",
                             json={"decision": "approve"})
        self.assertEqual(r.status_code, 404)
    ...
    def test_bad_decision_400(self):
        r = self.client.post("/api/review/knowledge:x/resolve",
                             json={"decision": "maybe"})
        self.assertEqual(r.status_code, 400)

    def test_unknown_type_400(self):
        r = self.client.post("/api/review/banana:x/resolve",
                             json={"decision": "approve"})
        self.assertEqual(r.status_code, 400)

    def test_missing_item_404(self):
        for t in ("knowledge", "reclassify", "suggestion", "duplicate"):
            r = self.client.post(f"/api/review/{t}:{uuid.uuid4()}/resolve",
                                 json={"decision": "approve"})
...
class ZipSuggestionTests(unittest.TestCase):
    ...
    def _explode(self, path: Path, work_id: str | None = None) -> list[str]:
        ...
        return _explode_zip_into_documents(parent["id"], path, work_id,
                                           "archive.zip", self.db)

    def _suggestions(self) -> list:
        with self.db._lock:
            return self.db._conn.execute(
                "SELECT * FROM suggestions WHERE kind='work_assignment'").fetchall()
