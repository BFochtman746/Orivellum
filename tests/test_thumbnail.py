...
def _make_test_jpeg(width: int, height: int, color=(200, 80, 80), quality: int = 85) -> str:
...
class TestMakeThumbnailB64(unittest.TestCase):
    ...
    def test_constants_exist_at_module_level(self):
        ...
        self.assertTrue(
            hasattr(conv, "_THUMBNAIL_MAX_PX"), "_THUMBNAIL_MAX_PX not found at module level"
        )
        self.assertTrue(
            hasattr(conv, "_THUMBNAIL_MAX_KB"), "_THUMBNAIL_MAX_KB not found at module level"
        )
    ...
    def test_large_landscape_image_resized_within_px_limit(self):
        ...
        self.assertLessEqual(max(thumb.size), _THUMBNAIL_MAX_PX, f"Thumb too large: {thumb.size}")
    ...
    def test_output_fits_within_kb_limit(self):
        ...
        self.assertLessEqual(
            len(raw),
            _THUMBNAIL_MAX_KB * 1024,
            f"Thumbnail too heavy: {len(raw)} bytes (limit {_THUMBNAIL_MAX_KB * 1024})",
        )
    ...
    def test_aspect_ratio_preserved(self):
        ...
        self.assertAlmostEqual(
            ratio, 2.0, delta=0.1, msg=f"Aspect ratio wrong: {w}×{h} = {ratio:.2f}"
        )
