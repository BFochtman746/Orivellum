...
class LicenseAckTest(unittest.TestCase):
    ...
    def test_unknown_model_404(self):
        ...
        with _client(app, db, cfg) as client:
            r = client.post("/api/studio/music/licenses/colossal_audio/ack",
                            json={"accepted": True}, headers=AUTH_HEADERS)
        self.assertEqual(r.status_code, 404)

    def test_ack_persists_and_revokes(self):
        app, db, cfg = _make_app(self.tmp.name)
        with _client(app, db, cfg) as client:
            r = client.post("/api/studio/music/licenses/musicgen/ack",
                            json={"accepted": True}, headers=AUTH_HEADERS)
            self.assertEqual(r.status_code, 200)
            ...
            r = client.post("/api/studio/music/licenses/musicgen/ack",
                            json={"accepted": False}, headers=AUTH_HEADERS)
            self.assertEqual(r.status_code, 200)
...
class GenerateGatesTest(unittest.TestCase):
    ...
    def _body(self, **over):
        base = {"prompt": "tense orchestral rise", "model": "stable_audio_open",
                "kind": "music", "duration_s": 20}
        base.update(over)
        return base
    ...
    def test_unconfigured_503(self):
        app, db, cfg = _make_app(self.tmp.name, music_gen_url="")
        with _client(app, db, cfg) as client:
            r = client.post("/api/studio/music/generate", json=self._body(),
    ...
    def test_unknown_model_404(self):
        app, db, cfg = _make_app(self.tmp.name, music_gen_url="http://127.0.0.1:9884")
        with _client(app, db, cfg) as client:
            r = client.post("/api/studio/music/generate",
                            json=self._body(model="riffusion"), headers=AUTH_HEADERS)
        self.assertEqual(r.status_code, 404)
    ...
    def test_empty_prompt_422(self):
        ...
        with _client(app, db, cfg) as client:
            r = client.post("/api/studio/music/generate",
                            json=self._body(prompt="   "), headers=AUTH_HEADERS)
    ...
    def test_bad_kind_422(self):
        ...
        with _client(app, db, cfg) as client:
            r = client.post("/api/studio/music/generate",
                            json=self._body(kind="jingle"), headers=AUTH_HEADERS)
        self.assertEqual(r.status_code, 422)
    ...
    def test_duration_over_model_cap_422(self):
        ...
        with _client(app, db, cfg) as client:
            r = client.post("/api/studio/music/generate",
                            json=self._body(duration_s=300), headers=AUTH_HEADERS)
        self.assertEqual(r.status_code, 422)
    ...
    def test_gate_order_license_checked_after_validation(self):
        ...
        with _client(app, db, cfg) as client:
            r = client.post("/api/studio/music/generate",
                            json=self._body(duration_s=0), headers=AUTH_HEADERS)
        self.assertEqual(r.status_code, 422)
...
class GenerateJobTest(unittest.TestCase):
    ...
    def _post_with_worker(self, client, body, post_mock):
        ...
        def inline_submit(fn, *args, **kwargs):
            fn(*args)
        with mock.patch("orivellum.api.executor.submit_bg", side_effect=inline_submit), \
             mock.patch("httpx.post", post_mock):
            return client.post("/api/studio/music/generate", json=body,
                               headers=AUTH_HEADERS)
    ...
    def test_success_writes_output_and_registers(self):
        ...
        db.set_setting("music_license_ack_stable_audio_open", "true")
        wav = b"RIFF" + b"\x00" * 64
        post_mock = mock.Mock(return_value=_FakeResponse(
            content=wav, headers={"X-Music-Engine": "stable_audio_open"}))
        ...
        with _client(app, db, cfg) as client:
            with mock.patch("orivellum.capabilities.persist.register_and_index",
                            side_effect=fake_register), \
                 mock.patch("orivellum.api.routes.studio._link_output_sync",
                            return_value="outputs/x.wav"):
                r = self._post_with_worker(
                    client,
                    {"prompt": "low braam", "model": "stable_audio_open",
                     "kind": "sfx", "duration_s": 4, "work_id": "w1"},
                    post_mock,
                )
        self.assertEqual(r.status_code, 200)
    ...
    def test_music_prefix_for_music_kind(self):
        ...
        with _client(app, db, cfg) as client:
            with mock.patch("orivellum.capabilities.persist.register_and_index"), \
                ...
                r = self._post_with_worker(
                    client,
                    {"prompt": "warm piano bed", "model": "musicgen",
                     "kind": "music", "duration_s": 30},
                    post_mock,
                )
            job = client.get(f"/api/studio/music/jobs/{r.json()['job_id']}",
    ...
    def test_registration_failure_is_partial_success(self):
        ...
        with _client(app, db, cfg) as client:
            with mock.patch("orivellum.capabilities.persist.register_and_index",
                ...
                r = self._post_with_worker(
                    client,
                    {"prompt": "eerie drone", "model": "stable_audio_open",
                     "kind": "music", "duration_s": 10},
                    post_mock,
                )
            job = client.get(f"/api/studio/music/jobs/{r.json()['job_id']}",
                             headers=AUTH_HEADERS).json()
    ...
    def test_sidecar_unreachable_job_error(self):
        ...
        with _client(app, db, cfg) as client:
            r = self._post_with_worker(
                client,
                {"prompt": "x", "model": "stable_audio_open", "kind": "music",
                 "duration_s": 10},
                post_mock,
            )
            job = client.get(f"/api/studio/music/jobs/{r.json()['job_id']}",
