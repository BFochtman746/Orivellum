...
class TestStudioStatusEndpoint(unittest.TestCase):
    ...
    def test_status_tts_strategy_names(self):
        ...
        # No-robot-voice policy: espeak-ng must never be listed as a strategy.
...
class TestTTSGracefulFailure(unittest.TestCase):
    ...
    def test_tts_no_neural_engine_returns_503(self):
        """When no neural engine is available we get a fast, structured 503 —
        never a robotic espeak fallback (owner policy)."""
        import time
        ...
        self.assertIn("kokoro_onnx", detail["strategies_tried"])
    ...
    def test_tts_never_invokes_espeak(self):
        """The synthesis path must never even attempt to launch espeak-ng."""
        import time
    ...
    def test_legacy_espeak_samples_are_never_served(self):
        ...
        espeak fallback must never be served — neither from a DB row marked
        ...
        db = _deps.get_db()
        fake_mp3 = b"ID3" + b"\x00" * 4000  # big enough to pass size checks

        # Case A: pre-DB filesystem file with unknown provenance, no DB row.
        fs_path = _get_sample_cache_path(cfg, "af_bella")
        fs_path.write_bytes(fake_mp3)

        # Case B: DB row explicitly marked as espeak-generated.
        db_path = _get_sample_cache_path(cfg, "am_adam")
        db_path.write_bytes(fake_mp3)
        _upsert_voice_sample_db(db, "am_adam", str(db_path), "espeak")
