...
class TestScoreEngine(unittest.TestCase):
    def test_concepts_scoring(self):
        ...
        case = {
            "expected_concepts": ["alpha", "beta", "gamma", "delta"],
...
class TestSeedingIdempotency(unittest.TestCase):
    ...
    def test_dynamic_version_bump_on_change(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            db.add_chunk(
                doc2["id"],
                "Mitochondria are the powerhouse organelle of "
...
class TestRetrievalRun(unittest.TestCase):
    def test_retrieval_run_end_to_end(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with db._lock:
                run = dict(
                    db._conn.execute("SELECT * FROM eval_runs WHERE id=?", (run_id,)).fetchone()
                )
...
class TestApiEndpoints(unittest.TestCase):
    def _fake_llm(self):
        ...
        def _fn(messages, **kwargs):
            return LLMResult(
                text="42", ok=True, model="fake", latency_ms=5, prompt_tokens=3, completion_tokens=1
            )
        ...
        for missing in ("benchmark_cases", "eval_runs"):
            with tempfile.TemporaryDirectory() as tmp:
                ...
                if missing == "benchmark_cases":
                    self.assertTrue(any(b["case_count"] > 0 for b in resp.json()["benchmarks"]))
    ...
    def test_benchmarks_error_detail_is_structured(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                "orivellum.api.routes.mcos._list_benchmarks_rows", side_effect=RuntimeError("boom")
            ):
                resp = client.get("/api/mcos/benchmarks")
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with db._lock:
                db._conn.execute(
                    ...
                    " VALUES(?,?,?,'running',0)",
                    (str(uuid.uuid4()), "reasoning", datetime.now(UTC).isoformat()),
                )
                db._conn.commit()
            resp = client.post("/api/mcos/run/reasoning")
...
class TestGroundingJudge(unittest.TestCase):
    ...
    def test_all_grounded(self):
        ...
        case = {
            "context": "Mitochondria produce adenosine triphosphate energy inside eukaryotic cells."
        }
...
class TestLLMJudge(unittest.TestCase):
    def _judge_returning(self, text, ok=True):
        ...
        def _fn(messages, **kwargs):
            return LLMResult(text=text if ok else None, ok=ok, model="fake", latency_ms=1)
    ...
    def test_good_json(self):
        ...
        with patch.object(
            mcos, "llm_call", self._judge_returning('{"score": 0.75, "reason": "close"}')
        ):
    ...
    def test_garbage_is_absent(self):
        ...
        with patch.object(mcos, "llm_call", self._judge_returning("no json here at all")):
            score, reason = mcos._llm_judge({"question": "q"}, "resp", None, None)
    ...
    def test_score_clamped(self):
        ...
        with patch.object(mcos, "llm_call", self._judge_returning('{"score": 2.5, "reason": "x"}')):
    ...
    def test_nan_score_is_absent(self):
        ...
        with patch.object(mcos, "llm_call", self._judge_returning('{"score": NaN, "reason": "x"}')):
            score, reason = mcos._llm_judge({"question": "q"}, "resp", None, None)
        self.assertIsNone(score)
        self.assertIsNone(reason)
    ...
    def test_infinity_score_is_absent(self):
        ...
        with patch.object(
            mcos, "llm_call", self._judge_returning('{"score": Infinity, "reason": "x"}')
        ):
            score, reason = mcos._llm_judge({"question": "q"}, "resp", None, None)
        self.assertIsNone(score)
        self.assertIsNone(reason)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def _fake(messages, **kwargs):
                ...
                if purpose == "mcos.judge":
                    return LLMResult(
                        text='{"score": 0.6, "reason": "ok"}', ok=True, model="fake", latency_ms=1
                    )
                return LLMResult(text="the answer is 42", ok=True, model="fake", latency_ms=1)
...
class TestRegressionGovernance(unittest.TestCase):
    def _failing_llm(self):
        ...
        def _fn(messages, **kwargs):
            if kwargs.get("purpose") == "mcos.judge":
                return LLMResult(
                    text='{"score": 0.0, "reason": "wrong"}', ok=True, model="fake", latency_ms=1
                )
            return LLMResult(text="the answer is 99", ok=True, model="fake", latency_ms=1)
    ...
    def test_ack_flow_and_404s(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with db._lock:
                meta = _json.loads(
                    db._conn.execute("SELECT meta FROM eval_runs WHERE id=?", (run_id,)).fetchone()[
                        0
                    ]
                )
            ...
            with db._lock:
                db._conn.execute(
                    ...
                    (ok_id, bid, datetime.now(UTC).isoformat()),
                )
                db._conn.commit()
    ...
    def test_nan_judge_keeps_meta_valid_json(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def _fake(messages, **kwargs):
                if kwargs.get("purpose") == "mcos.judge":
                    return LLMResult(
                        text='{"score": NaN, "reason": "x"}', ok=True, model="fake", latency_ms=1
                    )
                return LLMResult(text="the answer is 42", ok=True, model="fake", latency_ms=1)
...
class TestPromptRegistry(unittest.TestCase):
    ...
    def test_seed_creates_active_chat_base(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            seed_default_benchmarks(db)
            with db._lock:
                rows = db._conn.execute("SELECT * FROM prompts WHERE slot='chat.base'").fetchall()
    ...
    def test_create_list_activate_delete(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            resp = client.post(
                "/api/mcos/prompts",
                json={"slot": "chat.base", "name": "cand", "content": "candidate body"},
            )
            self.assertEqual(resp.status_code, 200)
    ...
    def test_prompt_run_excluded_from_regression(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(mcos, "llm_call", self._fake_llm()):
                pmeta = {"prompt_id": "p1", "prompt_role": "candidate", "prompt_slot": "chat.base"}
                run_id = mcos._create_run_row(db, cfg, bid, initial_meta=pmeta)
                mcos._execute_run(db, cfg, bid, run_id, system_prompt="X", run_meta=pmeta)
    ...
    def test_get_prompt_benchmark_status(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            client.post("/api/mcos/seed")
            resp = client.post(
                "/api/mcos/prompts", json={"slot": "chat.base", "name": "c", "content": "cand"}
            )
            pid = resp.json()["prompt"]["id"]
            # No runs yet.
    ...
    def test_benchmark_prompt_pairs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            client.post("/api/mcos/seed")
            resp = client.post(
                "/api/mcos/prompts", json={"slot": "chat.base", "name": "c", "content": "cand body"}
            )
            pid = resp.json()["prompt"]["id"]
            from orivellum.capabilities import mcos
...
class TestRagCalibration(unittest.TestCase):
    ...
    def test_sweep_end_to_end_no_writes(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with db._lock:
                ...
                row = dict(
                    db._conn.execute("SELECT * FROM rag_sweeps WHERE id=?", (sweep_id,)).fetchone()
                )
    ...
    def test_rag_config_and_apply(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Valid apply.
            resp = client.post(
                "/api/mcos/rag/apply", json={"target_words": 800, "overlap_words": 80}
            )
            self.assertEqual(resp.status_code, 200)
            ...
            resp = client.post(
                "/api/mcos/rag/apply", json={"target_words": 400, "overlap_words": 300}
            )
            self.assertEqual(resp.status_code, 400)
            # Invalid: target out of bounds.
            resp = client.post(
                "/api/mcos/rag/apply", json={"target_words": 50, "overlap_words": 10}
            )
            self.assertEqual(resp.status_code, 400)
    ...
    def test_rag_sweep_endpoint(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            db.add_chunk(
                doc["id"],
                "Alpha beta gamma delta epsilon marker phrase here "
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with db._lock:
                db._conn.execute(
                    ...
                    "VALUES(?, datetime('now','-45 minutes'), 'running')",
                    (sid,),
                )
                db._conn.commit()
            resp = client.get("/api/mcos/rag/sweeps")
...
class TestMultiSlotPrompts(unittest.TestCase):
    ...
    def test_judge_uses_active_prompt(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def _fake(messages, **kwargs):
                ...
                return LLMResult(
                    text='{"score":0.9,"reason":"ok"}', ok=True, model="fake", latency_ms=1
                )

            with patch.object(mcos, "llm_call", _fake):
                mcos._llm_judge({"question": "q", "expected_output": "42"}, "42", cfg, db)
    ...
    def test_judge_falls_back_when_no_registry(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def _fake(messages, **kwargs):
                seen["system"] = messages[0]["content"]
                return LLMResult(
                    text='{"score":0.5,"reason":"x"}', ok=True, model="fake", latency_ms=1
                )
    ...
    def test_create_unknown_slot_400(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            client = TestClient(app, raise_server_exceptions=True, headers=AUTH_HEADERS)
            resp = client.post(
                "/api/mcos/prompts", json={"slot": "bogus.slot", "name": "x", "content": "y"}
            )
            self.assertEqual(resp.status_code, 400)
    ...
    def test_benchmark_non_benchmarkable_slot_400(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            pid = client.post(
                "/api/mcos/prompts",
                json={"slot": "harvest.extract", "name": "h", "content": "extract {title} {chunk}"},
            ).json()["prompt"]["id"]
            resp = client.post(f"/api/mcos/prompts/{pid}/benchmark")
            self.assertEqual(resp.status_code, 400)
...
class TestRagReprocess(unittest.TestCase):
    ...
    def test_apply_triggers_reprocess(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(lib, "process_document", lambda **kw: None):
                resp = client.post(
                    "/api/mcos/rag/apply",
                    json={"target_words": 600, "overlap_words": 60, "reprocess_library": True},
                )
            self.assertEqual(resp.status_code, 200)
    ...
    def test_apply_without_reprocess_has_no_field(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            client = TestClient(app, raise_server_exceptions=True, headers=AUTH_HEADERS)
            resp = client.post(
                "/api/mcos/rag/apply", json={"target_words": 500, "overlap_words": 50}
            )
            self.assertEqual(resp.status_code, 200)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with db._lock:
                metas = [
                    _json.loads(
                        db._conn.execute(
                            "SELECT meta FROM eval_runs WHERE id=?", (rid,)
                        ).fetchone()["meta"]
                    )
                    for rid in hr["runs"]
                ]
            ...
            self.assertAlmostEqual(mcos._prev_finished_avg(db, "ph_bench", "nope"), 0.95)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with db._lock:
                final_meta = _json.loads(
                    db._conn.execute(
                        "SELECT meta FROM eval_runs WHERE id=?", (hr2["runs"][-1],)
                    ).fetchone()["meta"]
                )
            ...
            def _exec_wrapper(db_, cfg_, bid, rid, **kw):
                ...
                if calls["n"] == len(mcos._enabled_llm_benchmarks(db_)):
                    mcos._finalize_run(
                        ...
                        status="failed",
                        avg_score=None,
                        meta={**kw.get("run_meta", {}), "error": "boom"},
                    )
                    return rid
                return real_exec(db_, cfg_, bid, rid, **kw)

            with (
                patch.object(mcos, "llm_call", _bad),
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for s, info in mcos.PROMPT_SLOTS.items():
                if not info.get("benchmarkable"):
                    r = by_slot[s]
                    self.assertTrue(r.get("skipped"), f"slot {s!r} should be skipped, got: {r}")
                    self.assertTrue(
                        r.get("ok"), f"default prompt for {s!r} should pass structural validation"
                    )
                    self.assertEqual(r["runs"], [], f"no eval_run rows should exist for {s!r}")
    ...
    def test_regression_entry_carries_slot(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for entry in prompt_regs:
                self.assertIn("prompt_slot", entry, "regression entry must carry prompt_slot")
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with db._lock:
                db._conn.execute(
                    ...
                    (
                        "Extract knowledge about {title}. No chunk placeholder here.",
                        "harvest.extract",
                    ),
                )
                db._conn.commit()
            ...
            def _fake_run_all(db_, cfg_):
                ...
                return [
                    ...
                    },
                    {
                        "ok": True,
                        "slot": "chat.persona",
                        "slot_label": "Copilot persona (A-01)",
                        "prompt_name": "A-01 Copilot Persona",
                        "prompt_version": 1,
                        "runs": ["r2"],
                        "current_agg": 0.9,
                        "prev_agg": None,
                    ...
                    {
                        "ok": True,
                        "slot": "harvest.extract",
                        "slot_label": "Knowledge extraction",
                        "prompt_name": "Extract",
                        "prompt_version": 1,
                        "skipped": True,
                        "reason": "content valid",
                        "runs": [],
                    },
                    {
                        "ok": True,
                        "slot": "mcos.judge",
                        "slot_label": "MCOS judge",
                        "prompt_name": "Judge",
                        "prompt_version": 1,
                        "skipped": True,
                        "reason": "content valid",
                    ...
                    },
                    {
                        "ok": True,
                        "slot": "write.draft",
                        "slot_label": "Prose drafter",
                        "prompt_name": "Prose drafter",
                        "prompt_version": 1,
                        "skipped": True,
                        "reason": "content valid",
                        "runs": [],
                    },
                    {
                        "ok": True,
                        "slot": "write.critic",
                        "slot_label": "Adversarial editor",
                        "prompt_name": "Adversarial editor",
                        "prompt_version": 1,
                        "skipped": True,
                        "reason": "content valid",
                        "runs": [],
            ...
            for line in report:
                ...
                for hr in health_results:
                    if (hr.get("prompt_name") or "") in line and hr["slot"] not in matched:
                        matched.add(hr["slot"])
            self.assertEqual(matched, slots_covered, f"report lines: {report!r}")
