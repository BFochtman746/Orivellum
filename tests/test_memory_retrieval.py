...
def _seed_fact(
    ...
    db._conn.execute(
        """INSERT INTO user_memory
           ...
           VALUES (?,?,?,?,?,NULL,?,?,?,?)""",
        (mid, key, value, memory_type, now, now, now, source_conv_id, source_evidence_id),
    )
    db._conn.commit()
    ...
    try:
        db._conn.execute(
            ...
            " SELECT rowid, key, value, id FROM user_memory WHERE id=?",
            (mid,),
        )
        db._conn.commit()
    except Exception:
...
class TestSchemaV101(unittest.TestCase):
    ...
    def test_user_memory_fts_has_key_and_value_columns(self):
        ...
        try:
            self.db._conn.execute("SELECT * FROM user_memory_fts LIMIT 0").fetchall()
...
class TestSearchMemoriesLexical(unittest.TestCase):
    ...
    def test_limit_respected(self):
        for i in range(10):
            self.db.upsert_memory_fact(f"lim_key_{i}", "common search term across all")
...
class TestSearchMemoriesGraph(unittest.TestCase):
    ...
    def test_finds_fact_mentioning_entity(self):
        ...
        self.db.upsert_memory_fact("lang_pref", "I use Python daily for data work")
        hits = self.db.search_memories_graph("Python", limit=10)
        keys = [h["key"] for h in hits]
        self.assertIn("lang_pref", keys, "Graph channel must find fact mentioning the entity name")
    ...
    def test_traverses_one_hop_neighbour(self):
        ...
        self.db.upsert_memory_fact("ml_stack", "I am learning DeepLearning frameworks")
        ...
        keys = [h["key"] for h in hits]
        self.assertIn(
            "ml_stack", keys, "Graph channel must traverse 1-hop to surface neighbour-entity facts"
        )
    ...
    def test_excludes_superseded_facts(self):
        ...
        self.assertEqual(len(old_hits), 0, "Graph channel must exclude superseded rows")
...
class TestMergeLayer(unittest.TestCase):
    ...
    def test_dedup_by_id(self):
        ...
        self.assertEqual(
            ids.count("id-1"), 1, "Items appearing in multiple channels must be deduplicated"
        )
...
class TestSearchMemoriesIntegration(unittest.TestCase):
    ...
    def test_lexical_channel_surfaces_keyword_match(self):
        ...
        self.db.upsert_memory_fact("coffee_habit", "I drink espresso every morning before work")
        ...
        keys = [h["key"] for h in hits]
        self.assertIn(
            "coffee_habit", keys, "Lexical channel must surface fact when semantic channel is empty"
        )
    ...
    def test_graph_channel_surfaces_entity_mention(self):
        ...
        self.db._conn.execute(
            "INSERT OR IGNORE INTO entities(id,name,kind,aliases,meta,created_at)"
            " VALUES(?,?,?,'[]','{}',?)",
            (eid, "PostgreSQL", "technology", now),
        )
        self.db._conn.commit()
        self.db.upsert_memory_fact("db_choice", "I prefer PostgreSQL for relational data")
        with (
        ...
        keys = [h["key"] for h in hits]
        self.assertIn(
            "db_choice", keys, "Graph channel must surface fact mentioning the entity name"
        )
    ...
    def test_retrieval_source_field_present(self):
        ...
        for h in hits:
            self.assertIn("retrieval_source", h, "Each result must have a retrieval_source field")
            self.assertIn(h["retrieval_source"], {"semantic", "lexical", "graph", "multi"})
    ...
    def test_all_channels_fail_returns_empty(self):
        ...
        with (
            patch("orivellum.capabilities.memory._channel_semantic", side_effect=_raise),
    ...
    def test_multi_source_when_in_both_channels(self):
        ...
        try:
            self.db._conn.execute(
                "INSERT OR IGNORE INTO entities(id,name,kind,aliases,meta,created_at)"
                " VALUES(?,?,?,'[]','{}',?)",
                (eid, "Docker", "technology", now),
            )
            self.db._conn.commit()
        except Exception:
            pass
        self.db.upsert_memory_fact(
            "container_tool", "I use Docker for all my containerisation work"
        )
        with patch("orivellum.capabilities.memory._channel_semantic", return_value=[]):
            from orivellum.capabilities.memory import search_memories
...
class TestGetMemoryEndpoint(unittest.TestCase):
    ...
    def test_with_q_returns_hybrid_results(self):
        ...
        with (
            ...
            result = asyncio.run(get_memory(q="Neovim editor", include_evidence=False))
    ...
    def test_q_empty_string_falls_back_to_all_facts(self):
        ...
        with _patch("orivellum.api.routes.conversations.get_db", return_value=self.db):
            result = asyncio.run(get_memory(q="  ", include_evidence=False))
