    ...
    *,
    ...
    with db._lock:
        db._conn.execute(
            ...
            (
                rid, f"enc-{rid}", f"hash-{rid}",
                subject, "Sender Name", sender_domain,
                received_at, lifecycle_state, aid,
                now, now,
            ),
        )
        db._conn.execute(
            ...
            (
                aid, rid, attention_level, needs_reply,
                "Test rationale", "REVIEW", 0.9,
                0, 0, "test-model",
                "[]", now,
            ),
        )
...
class TestRecencyCap:
    ...
    def test_recent_message_included_within_cap(self, tmp_path):
        ...
        rid = _insert_record(
            db, received_at=_iso(1), attention_level="high",
    ...
    def test_cap_disabled_when_days_zero(self, tmp_path):
        ...
        assert rid in ids, (
            "Expected old message to appear when days=0 (cap disabled)"
        )
        db.close()
    ...
    def test_boundary_message_at_exactly_cap_is_included(self, tmp_path):
        ...
        rid = _insert_record(
            db, received_at=_iso(29.9), attention_level="medium",
        )
    ...
    def test_ordering_high_before_medium(self, tmp_path):
        ...
        medium_id = _insert_record(
            db, received_at=_iso(1), attention_level="medium",
            subject="Medium mail",
        )
        high_id = _insert_record(
            db, received_at=_iso(2), attention_level="high",
    ...
    def test_limit_respected(self, tmp_path):
        ...
        assert len(results) == 3, (
            f"Expected exactly 3 results (limit=3), got {len(results)}"
        )
        db.close()
    ...
    def test_mixed_old_and_recent_only_recent_returned(self, tmp_path):
        ...
        old_id = _insert_record(
            db, received_at=_iso(45), attention_level="high",
            subject="Old urgent mail",
        )
        recent_id = _insert_record(
            db, received_at=_iso(5), attention_level="high",
            subject="Recent urgent mail",
        )
