...
**Cold-install parsing rule:** uv's non-TTY output has no `Resolved`/`Prepared` lines — the stage machine must key on `Downloading`/` Downloaded` (leading space)/`Installed` lines, and completion lines must advance the detail or the UI sits on one package name for the whole torch fetch.
