...
- **The DB transition path is the authority, never the caller.** Certification changes go through one write path that re-reads the current contract inside the transaction, aggregates the complete disposition record scoped to the current shadow epoch (evidence resets every time an instrument enters shadow — advisory-era or prior-contract verdicts never promote), and refuses below the declared bar. Lost CAS race → RuntimeError → 409.
