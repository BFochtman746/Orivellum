...
## Schedule-run admission (Run now + tick)
- All schedule-linked runnable ops are created ONLY through `_admit_and_create` in scheduler.py: admission check + INSERT under the single DB RLock. Any running op OR any pending op blocks admission.
- **Why:** separate check-then-create windows let a manual "Run now", a tick, and the /start endpoint double-start; the pending state means "about to be claimed", so it must count as live.
- **How to apply:** any new path that starts an operation for a schedule must go through `_admit_and_create`; any path that creates a pending op and fails to submit it must CAS it to failed (`store.fail_pending_operation`) or admission wedges forever. Tick un-claims a lost occurrence via `_unclaim_occurrence` (CAS-fenced on the exact claimed values) so it is deferred, never lost. Manual /start deliberately stays outside the quiet rule (user-priority).
