...
- **Atomic claim rule:** every resolver (and defer) must claim ownership with a conditional write (rowcount check) before applying side effects; losers get 409/404.
  **Why:** stale UI cards and concurrent requests otherwise overturn finished human decisions or double-apply effects (e.g. two Works created from one suggestion); read-then-act sequences are racy under the threaded route pool.
