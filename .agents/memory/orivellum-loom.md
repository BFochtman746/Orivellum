...
- **Grounding must reach the judges, not just the assembler.** Critic and narrator prompts must carry the horizon-restricted persona, canon facts, world state, and previous closing passage — assembling context and then not feeding it to a judge silently voids the consistency guarantee. Completion review rejected exactly this once; prompt-content tests now pin it.
- **Narrator selection is strict.** Missing/malformed/empty selection → refuse the run; never default to "all accepted actions". Only selected actions' world updates commit.
