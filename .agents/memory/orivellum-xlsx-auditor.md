...
- 3-D sheet spans (`=SUM(S1:S3!A1)`) must expand into one exact area per member sheet using workbook sheet order (':' is illegal in sheet names, so a ':' prefix is always a span); without the sheet list, refuse and disclose — a composite "S1:S3" sheet key silently fabricates a nonexistent node and falsely marks the real per-sheet inputs unread.
