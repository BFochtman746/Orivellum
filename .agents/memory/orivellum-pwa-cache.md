...
- **Chunking rule:** NEVER split node_modules into multiple manual chunks (react/radix/misc). Circular inter-chunk imports broke evaluation order in production → "can't access property 'forwardRef' of undefined" → blank page in every browser; dev mode hides it (no chunking). Keep ONE vendor chunk (one-way index→vendor) + `maximumFileSizeToCacheInBytes` raised to 4 MiB. Verify by grepping built chunks: vendor must import nothing relative.
