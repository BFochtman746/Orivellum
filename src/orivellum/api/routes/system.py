...
def list_user_memory():
    ...
    try:
        with db._lock:
            try:
                # v99+ schema: includes source_evidence_id
                ...
                try:
                    # v98 fallback (no source_evidence_id yet)
                    rows = db._conn.execute(
                        """SELECT id, key, value, memory_type, valid_from, valid_to,
...
def get_audio_enhance_setting():
    ...
    return {
        ...
        "setup_progress": pr.get("setup_progress"),
...
def probe_audio_enhance():
    ...
    return {
        ...
        "setup_progress": result.get("setup_progress"),
        "error": result["error"],
