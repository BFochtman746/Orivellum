...
def run_forge_job(db: object, cfg: object, project_id: str, job_id: str) -> None:
    ...
    try:
        ...
        db.update_forge_job(
            job_id, status="failed", completed_at=_now(), meta=json.dumps({"error": str(exc)})
        )
...
def _run_design(db, cfg, project, job, event):
    ...
    db.update_forge_job(
        job["id"], status="awaiting_approval", plan_job_id=plan_job_id, completed_at=_now()
    )
...
def _run_build(db, cfg, project, job, event):
    ...
    selected_id = job.get("meta", {}) and json.loads(job.get("meta", "{}")).get(
        "selected_concept_id"
    )
    ...
    status = (
        ...
        if gate_summary["status"] == "conditional"
        else "blocked"
    )
    ...
    event(
        "build_complete",
        f"Build finished — gate status: {gate_summary['status'].upper()}.",
        gate_summary,
    )
...
def _run_verify(db, cfg, project, job, event):
    ...
    db.update_forge_job(job["id"], status=status, target_job_id=target_job_id, completed_at=_now())
    event(
        "verify_complete",
        f"Verification complete — {gate_summary['status'].upper()}.",
        gate_summary,
    )
...
def _run_repair(db, cfg, project, job, event):
    ...
    summary = run_builder(
        ...
        instruction=(
            job.get("instruction")
            or "Fix quality-gate failures from the previous build. Do not add unrelated features."
        ),
        on_event=event,
    ...
    db.update_forge_job(
        ...
        completed_at=_now(),
        meta=json.dumps({"repair_summary": summary, "gate_status": gate_summary["status"]}),
    )
