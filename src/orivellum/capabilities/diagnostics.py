...
def _check_db_integrity(db: OrivellumDB) -> list[dict]:
    ...
    checks.append(_check("Schema version", OK, f"v{ver}", "Database schema is current"))
    ...
    if ic_result == ["ok"]:
        checks.append(
            _check("DB integrity_check", OK, "ok", "SQLite reports no structural corruption")
        )
    else:
    ...
    checks.append(
        _check("DB quick_check", OK if qc == "ok" else ERROR, qc, "Fast structural check")
    )
...
def _check_orphans(db: OrivellumDB) -> list[dict]:
    ...
    try:
        ...
        checks.append(
            _check(
                "Orphaned documents (no objects row)",
                status,
                n,
                f"{n} documents missing their objects record — these may be inaccessible"
                if n
                else "Clean",
            )
        )
    ...
    try:
        ...
        checks.append(
            _check(
                "Orphaned knowledge items (missing work)",
                status,
                n,
    ...
    try:
        ...
        checks.append(
            _check(
                "Orphaned vectors (chunk type, missing chunk)",
                status,
                n,
        ...
        checks.append(
            _check("Orphaned vectors (chunk type, missing chunk)", WARN, "error", str(exc))
        )
...
def _check_stuck_records(db: OrivellumDB) -> list[dict]:
    ...
    try:
        ...
        checks.append(
            _check(
                "Documents stuck in 'imported'",
                status,
                n,
                f"{n} docs have been importing for >10 min — reprocess from Library"
                if n
                else "None stuck",
            )
        )
    ...
    try:
        ...
        checks.append(
            _check(
                "Pipeline artifacts stuck in 'running'",
                status,
                n,
    ...
    try:
        ...
        checks.append(
            _check(
                "Open high/critical governance findings",
                status,
                n,
...
def _check_configuration(db: OrivellumDB, cfg: OrivellumConfig) -> list[dict]:
    ...
    checks.append(
        _check(
            "Data directory",
            OK if (exists and writable) else ERROR,
            data_dir,
            "exists and writable"
            if (exists and writable)
            else ("exists but not writable" if exists else "directory missing"),
        )
    )
...
def _check_services(cfg: OrivellumConfig) -> list[dict]:
    ...
    try:
        ...
        checks.append(
            _check(
                ...
                "LLM inference server responded"
                if ok
                else "Unexpected status — AI features may fail",
            )
        )
    except Exception as exc:
        checks.append(
    ...
    checks.append(
        _check(
            "espeak-ng (unused, informational)",
            OK if espeak else WARN,
            espeak or "not found",
            "Present but not used for speech — neural engines only"
            if espeak
            else "Not installed — fine; robotic fallback is disabled by policy",
        )
    )
    ...
    checks.append(
        _check(
            "OCR (tesseract)",
            OK if tess else WARN,
            tess or "not found",
            "OCR available for image/scanned PDF extraction"
            if tess
            else "Install tesseract for image text extraction",
        )
    )
    ...
    checks.append(
        _check(
            ...
            OK if ff else WARN,
            ff or "not found",
            "ffmpeg available for audio format conversion"
            if ff
            else "Install ffmpeg for TTS audio encoding",
        )
    )
...
def _check_data_quality(db: OrivellumDB) -> list[dict]:
    ...
    try:
        ...
        checks.append(
            _check(
                "Documents missing extracted text",
                status,
                n,
                f"{n} docs marked ready but have no text — reprocess them"
                if n
    ...
    try:
        ...
        checks.append(
            _check(
                "AI knowledge items awaiting review",
                INFO,
                n,
    ...
    try:
        ...
        checks.append(
            _check(
                "Governance queue items pending",
                status,
                n,
        ...
        checks.append(
            _check(
                "Governance queue items pending", INFO, "—", "Table not present or no pending items"
            )
        )
        ...
        try:
            ...
            checks.append(
                _check(
                    ...
                    last_run_raw,
                    f"Ran {age_label}"
                    + (" — overdue (expected every ~24 h)" if age_h > 36 else ""),
                )
            )
        except Exception:
    ...
    try:
        with db._lock:
            rows = db._conn.execute(
                "SELECT status, COUNT(*) as n FROM book_pipelines GROUP BY status ORDER BY status"
            ).fetchall()
        if rows:
            summary = ", ".join(f"{r['status']}={r['n']}" for r in rows)
            checks.append(
                _check(
                    "Book pipelines by stage",
                    INFO,
                    summary,
            ...
            checks.append(
                _check("Book pipelines by stage", INFO, "none", "No pipelines created yet")
            )
    except Exception as exc:
...
def run_full_diagnostic(
    ...
    summary["health"] = "error" if summary["error"] > 0 else "warn" if summary["warn"] > 0 else "ok"
