    ...
    try:
        ...
        if _ce_on and _win > 1:
            ...
            if _scores is not None:
                _head = sorted(zip(all_passages[:_win], _scores), key=lambda t: t[1], reverse=True)
...
def _search_youtube(query: str) -> list[dict]:
    ...
    with_ids = [(r, vid) for r in results if (vid := _youtube_video_id(r.get("url", "")))]
    if with_ids:
        with ThreadPoolExecutor(max_workers=min(len(with_ids), 6)) as pool:
            futs = {pool.submit(_fetch_youtube_transcript, vid): r for r, vid in with_ids}
...
def research_web(
    ...
    *,
    ...
    with ThreadPoolExecutor(max_workers=12) as pool:
        ...
        _submit(_make_web_task(query, search_depth, include_raw, days), f"web:{query[:40]}")
        ...
        if SearchMode.ACADEMIC in resolved_modes:
            _submit(_make_academic_task(query, search_depth, include_raw), f"academic:{query[:40]}")

        if SearchMode.BIBLICAL in resolved_modes:
            # One biblical-domain search per main query (primary only — avoids over-fetching)
            _submit(_make_biblical_task(query, search_depth, include_raw), f"biblical:{query[:40]}")
        ...
        if n_variants > 0 and llm_call_fn is not None:
            planner_future = pool.submit(_plan_queries, query, n_variants, llm_call_fn, is_biblical)
            ...
            for v in variants:
                ...
                queries.append(v)
                _submit(_make_web_task(v, search_depth, include_raw, days), f"web:{v[:40]}")
