...
def hr():
    return HRFlowable(width="100%", thickness=0.5, color=LTGREY, spaceAfter=6, spaceBefore=6)


def thin_hr():
    return HRFlowable(width="100%", thickness=0.25, color=LTGREY, spaceAfter=4, spaceBefore=4)
...
def header_footer(canvas, doc):
    ...
    canvas.drawRightString(
        W - 0.5 * inch, H - 0.35 * inch, doc.title if hasattr(doc, "title") else ""
    )
    # Footer bar
    canvas.setFillColor(NAVY)
...
class OrivellumDoc(BaseDocTemplate):
    def __init__(self, filename):
        ...
        body_frame = Frame(0.65 * inch, 0.65 * inch, W - 1.3 * inch, H - 1.5 * inch, id="body")
        cover_frame = Frame(0.65 * inch, 1.0 * inch, W - 1.3 * inch, H - 2.0 * inch, id="cover")
...
def build_story():
    ...
    story.append(
        HRFlowable(
            width="60%", thickness=1, color=GOLD, spaceAfter=16, spaceBefore=0, hAlign="CENTER"
        )
    )
    story.append(
        p(
    ...
    arch_data = [
        ...
        [
            ...
            "REST + streaming SSE endpoints, background pipeline, nightshift daemon",
        ],
        [
            "Database",
            "SQLite (WAL mode) · FTS5 · custom migrations",
            "All documents, knowledge, conversations, embeddings, settings",
        ],
        [
            "Document pipeline",
            "pypdf / pdfplumber · Tesseract OCR · python-docx · openpyxl · markitdown",
            "Extract, chunk, embed, harvest knowledge from every file type",
        ],
        [
            "AI gateway",
            "Local OpenAI-compatible server (Lemonade or Ollama)",
            "Chat, extraction, embeddings, TTS, ASR — all local",
        ],
        [
            "Web frontend",
            "React 19 · Vite 7 · TanStack Query · Tailwind CSS 4 · Wouter",
            "Full workspace UI — every page in this manual",
        ],
        [
            "Search",
            "SQLite FTS5 (lexical) + local embeddings (semantic) + BM25 hybrid",
            "Retrieval-augmented generation for chat context",
        ],
        [
            "Web research",
            "Tavily API · YouTube Transcript API · RRF + BM25 pipeline",
            "Multi-mode internet research with citation assembly",
        ],
    ]
    t = Table(
        [
            [Paragraph(c, S["table_header"] if i == 0 else S["table_cell"]) for c in row]
    ...
    concepts = [
        (
            "<b>Work</b>",
            "A research or writing project. The core organizational unit. A Work links documents, knowledge items, conversations, tasks, chapters, and book pipeline state.",
        ),
        (
            "<b>Document</b>",
            "Any uploaded file — PDF, DOCX, XLSX, CSV, Markdown, audio, image, ZIP archive. Documents are processed through the extraction pipeline to produce text, chunks, and knowledge items.",
        ),
        (
            "<b>Knowledge Item</b>",
            "A structured fact or claim extracted from a document by rule-based and/or LLM-based harvesting. Knowledge items have confidence scores, source citations, and review status.",
        ),
        (
            "<b>Conversation</b>",
            "An AI chat session. Can be linked to a Work so the AI draws context from that Work's knowledge when answering.",
        ),
        (
            "<b>Nightshift</b>",
            "A background daemon that runs nightly (default 3 AM). It vacuums the database, processes stuck documents, refreshes embeddings, detects knowledge gaps, and runs 20+ maintenance passes.",
        ),
        (
            "<b>MCOS</b>",
            "Model Calibration &amp; Observability System. Tracks every LLM call (model, latency, tokens, success/failure), runs benchmark suites, and maintains a prompt health registry for regression detection.",
        ),
        (
            "<b>Embeddings</b>",
            "Vector representations of text used for semantic search. Stored locally in SQLite. Requires an embedding model running on your local LLM server.",
        ),
        (
            "<b>Readiness</b>",
            "A document's processing state: <i>imported</i> (queued), <i>ready</i> (processed), <i>error</i> (extraction failed), <i>no_text</i> (nothing readable), <i>draft</i> (manually created).",
        ),
    ]
    ...
    sys_data = [
        ...
        [
            "FFmpeg",
            "Audio conversion for TTS/ASR pipeline",
            "sudo apt-get install ffmpeg",
            "brew install ffmpeg",
        ],
        [
            "espeak-ng",
            "Fallback text-to-speech (optional)",
    ...
    story.append(
        p("All variables are optional — defaults work for a local single-user installation.")
    )
    story.append(sp(4))
    env_vars = [
    ...
    server_data = [
        ...
        [
            ...
            "AMD Ryzen AI / NPU acceleration; native Windows; recommended for the hardware this system was designed for",
        ],
        [
            "Ollama",
            "ollama.com/download",
            ...
            "Easy cross-platform setup; wide model library; Nvidia/CPU fallback",
        ],
        [
            "LM Studio",
            "lmstudio.ai",
            ...
            "GUI-first; good for exploring models before committing",
        ],
        [
            ...
            "varies",
            "any /v1 path",
            "Linux servers, high-throughput production use",
        ],
    ]
    t3 = Table(
        [
    ...
    model_data = [
        ...
        [
            "Workhorse",
            "workhorse_model",
            "Chat, summarisation, general reasoning, web research synthesis",
            "Qwen3.6-35B-A3B-GGUF (fast MoE, vision built in)\nQwen3-30B-A3B-Instruct-2507-GGUF",
        ],
        [
            "Reasoner",
            "reasoner_model",
            "Complex multi-step reasoning tasks, verification, deep analysis",
            "gpt-oss-120b-mxfp-GGUF (best local, needs 128 GB RAM)\ngpt-oss-20b-mxfp4-GGUF (smaller machines)",
        ],
        [
            "Coder",
            "coder_model",
            "Code generation, document workshop, structured output",
            "Qwen3-Coder-30B-A3B-Instruct-GGUF (256K context)\nQwen3-Coder-Next-GGUF (~48 GB)",
        ],
        [
            "Embedder",
            "embedder_model",
            "Semantic search vectors for all documents and knowledge",
            "Qwen3-Embedding-8B-GGUF (recommended)\nnomic-embed-text-v2-moe-GGUF (lightweight)",
        ],
        [
            "Vision",
            "vision_model",
            "OCR of scanned documents and images (fallback to Tesseract)",
            "Qwen3.6-35B-A3B-GGUF (same as Workhorse)\n(leave empty to use Tesseract)",
        ],
        [
            "Reranker",
            "reranker_model",
            "Re-ranks search results for better retrieval quality (optional)",
            "bge-reranker-v2-m3-GGUF (optional)",
        ],
    ]
    t4 = Table(
        [
    ...
    story.append(
        p(
            "Create or edit <b>config.yaml</b> in the project root. Everything is optional — "
...
ollama pull qwen3-coder:30b"""
    ...
    story.append(
        bullet("<b>Semantic Search</b> — shows embedding model status and circuit breaker state")
    )
    ...
    story.append(
        bullet(
            "<b>Telemetry</b> — every LLM call is logged: model name, purpose, latency, input/output tokens, success/failure, error"
        )
    )
    story.append(
        bullet(
            "<b>Prompt registry</b> — system prompts are versioned; regressions are detected when a prompt's quality drops"
        )
    )
    story.append(
        bullet(
            "<b>Benchmark suites</b> — nightly runs test model quality against known-correct Q&amp;A pairs"
        )
    )
    story.append(
        bullet(
            "<b>MCOS page</b> — view call history, model health, benchmark results, and prompt health at /mcos"
        )
    )
    story.append(sp(4))
    story.append(
    ...
    nightshift_passes = [
        (
            "DB Optimisation",
            "Runs SQLite integrity_check, WAL checkpoint, ANALYZE, and conditional VACUUM (only if >30% of pages are free)",
        ),
        ...
        (
            "Orphan cleanup",
            "Deletes knowledge items, chunks, and vectors whose parent documents were deleted",
        ),
        (
            "Stuck-document recovery",
            "Retries up to 5 documents per run that have been stuck in 'imported' or 'error' state for more than 10 minutes",
        ),
        (
            "VLM re-extraction",
            "Re-processes scanned PDFs and images that landed in no_text using the vision model (if configured)",
        ),
        (
            "Audio retranscription",
            "Re-runs speech-to-text on audio files that failed or have outdated transcripts",
        ),
        (
            "Sparse doc harvest",
            "Runs LLM knowledge harvest on documents that have fewer than 3 knowledge items",
        ),
        (
            "Gap analysis",
            "Detects knowledge gaps in active Works and writes results to the gap cache",
        ),
        ("Evidence rescore", "Re-scores knowledge item confidence and detects contradictions"),
        (
            "Chunk-prefix backfill",
            "Adds contextual prefixes to chunks missing them, then re-embeds",
        ),
        (
            "Embedding backfill",
            "Generates embeddings for chunks and knowledge items that are missing vectors",
        ),
        (
            "Stats refresh",
            "Updates work-level aggregate statistics (document count, knowledge count, etc.)",
        ),
        (
            "MCOS benchmarks",
            "Runs model quality benchmarks and checks prompt health for regressions",
        ),
        ...
        (
            "Version suggestions",
            "Suggests version relationships between similar documents in the same Work",
        ),
        ("Topic clustering", "Groups knowledge items into topic clusters for the Topics page"),
        (
            "Memory management",
            "Expires old working memory, deduplicates, promotes episodic→semantic memory",
        ),
    ]
    ...
    story.append(
        p("Every document has a <b>readiness</b> field that shows where it is in the pipeline:")
    )
    story.append(sp(4))
    readiness_data = [
        ...
        [
            "imported",
            "Queued for extraction — waiting for the pipeline to pick it up",
            "Wait up to 2 minutes. If it stays 'imported' for more than 10 minutes, the pipeline may be stuck — use Reprocess.",
        ],
        [
            "ready",
            "Fully extracted, chunked, embedded, and knowledge-harvested",
            "Nothing — this is the healthy terminal state.",
        ],
        [
            "error",
            "Extraction failed with an error message",
            "Click the document → see the error detail → fix the root cause (missing tool, corrupt file) → click Reprocess.",
        ],
        [
            "no_text",
            "File processed but no readable text found",
            "For scanned PDFs: configure a vision model. For images: check Tesseract is installed. Then Reprocess.",
        ],
    ...
    t5 = Table(
        [
            [Paragraph(c, S["table_header"] if i == 0 else S["table_cell"]) for c in row]
            for i, row in enumerate(readiness_data)
        ],
    ...
    story.append(
        bullet(
            "Go to <b>Library</b> → click the document → click <b>Reprocess</b> in the document detail header"
        )
    )
    story.append(bullet("Or via API: <code>POST /api/library/{doc_id}/reprocess</code>"))
    story.append(
        bullet("Use <code>?force=true</code> to reprocess even if the document is already ready")
    )
    story.append(sp(4))
...
uv run python scripts/run_diagnostics.py --vacuum"""
    ...
    story.append(
        bullet("Go to <b>System</b> → Semantic Search card → click <b>Probe Embeddings</b>")
    )
    story.append(
        bullet("If the probe succeeds but dimensions don't match: click <b>Reindex Embeddings</b>")
    )
    story.append(
        bullet(
    ...
    story.append(
        bullet(
            "<b>Custodian Header</b> — time-of-day greeting, pending task count, and quick-action buttons"
        )
    )
    story.append(
        bullet(
            "<b>Suggestions Panel</b> — AI-generated suggestions for your active Work (tasks, research questions, gaps to fill)"
        )
    )
    story.append(
        bullet(
            "<b>Gap Analysis Panel</b> — knowledge gaps detected across your Works, with suggested actions"
        )
    )
    story.append(
        bullet(
            "<b>Scorecard</b> — aggregate health metrics: total documents, knowledge items, pending reviews, active conversations"
        )
    )
    story.append(
        bullet(
            "<b>Active Works Cards</b> — your in-progress Works with progress bars and quick navigation"
        )
    )
    story.append(
        bullet(
            "<b>Recent Documents</b> — last-uploaded or last-modified documents with readiness indicators"
        )
    )
    story.append(
        bullet(
            "<b>Recent Conversations</b> — your most recent AI chat sessions with model and timestamp"
        )
    )
    story.append(
        bullet(
            "<b>Activity Feed</b> — timestamped log of everything that has happened across the system"
        )
    )
    ...
    story.append(
        bullet("The scorecard turning yellow or red on any metric means something needs attention")
    )
    story.append(sp(8))
    ...
    story.append(
        bullet(
            "<b>Works Grid</b> — cards showing title, type, status, document count, knowledge count, and a completion progress bar"
        )
    )
    ...
    story.append(h3("How to Use"))
    story.append(
        bullet("Click <b>Create Work</b> → fill in title, type, and optional description → Save")
    )
    story.append(bullet("Click any Work card to open the full Work workspace"))
    story.append(
        bullet("Use the filter chips to show only Fiction, Research, or a specific status")
    )
    story.append(sp(8))
    ...
    story.append(
        bullet(
            "<b>Overview</b> — Work metadata, description, statistics bar (document count, knowledge count, conversations), book health card, and quick-chat button"
        )
    )
    story.append(
        bullet(
            "<b>Documents</b> — all documents linked to this Work with readiness states, upload button, and unlink option (hover a doc to reveal the X button)"
        )
    )
    story.append(
        bullet(
            "<b>Knowledge</b> — all extracted knowledge items with confidence scores, review status (AI-auto vs rule-based), approve/reject thumbs, and search"
        )
    )
    story.append(
        bullet(
            "<b>Book</b> — the book production pipeline: chapter ordering, pipeline state machine (Draft → Research → Writing → Editing → Final), and the book intelligence view"
        )
    )
    story.append(
        bullet(
            "<b>Brainstorm</b> — AI brainstorming canvas for generating ideas, outlines, and research questions linked to this Work"
        )
    )
    story.append(
        bullet(
            "<b>Tasks</b> — task list linked to this Work; create, complete, and prioritise tasks"
        )
    )
    story.append(
        bullet(
            "<b>Intelligence</b> — knowledge quality dashboard: gaps, contradictions, confidence distribution, chapter coverage"
        )
    )
    ...
    story.append(
        bullet("Use Quick Chat (Overview tab) to open a conversation pre-linked to this Work")
    )
    story.append(
        bullet("Advance the book pipeline (Book tab) as you complete each production stage")
    )
    story.append(sp(8))
    ...
    story.append(
        bullet(
            "<b>Confidence Summary</b> — distribution of knowledge item confidence scores (high / medium / low)"
        )
    )
    story.append(
        bullet(
            "<b>Health Score</b> — aggregate health: completeness, evidence quality, contradiction rate"
        )
    )
    story.append(
        bullet(
            "<b>Knowledge Gaps</b> — topics that are mentioned but not well-evidenced, with suggested research questions"
        )
    )
    story.append(
        bullet("<b>Contradictions</b> — pairs of knowledge items that say conflicting things")
    )
    story.append(
        bullet(
            "<b>Chapter Coverage</b> — for books: which chapters have strong knowledge backing vs thin coverage"
        )
    )
    story.append(
        bullet(
            "<b>Knowledge Item List</b> — all items filterable by confidence band, source, and review status"
        )
    )
    story.append(h3("How to Use"))
    story.append(
    ...
    story.append(
        bullet(
            "<b>Conversations Sidebar</b> — list of all conversations with timestamps, archived state, and rename/archive actions"
        )
    )
    story.append(
        bullet(
            "<b>Model &amp; Mode Controls</b> — choose the active model, toggle Deep Mode (uses the Reasoner model), connectivity indicator"
        )
    )
    story.append(
        bullet(
            "<b>Message Thread</b> — streaming AI responses with markdown rendering, code syntax highlighting, and copy-to-clipboard"
        )
    )
    story.append(
        bullet(
            "<b>Composer</b> — message input with file attachment, internet search toggle, and send button"
        )
    )
    story.append(
        bullet(
            "<b>Sources Panel</b> — knowledge items and web sources that were injected as context for the current response"
        )
    )
    story.append(
        bullet(
            "<b>Work Badge</b> — when the conversation is linked to a Work, a badge links back to that Work's detail page"
        )
    )
    story.append(
        bullet("<b>Files Panel</b> — documents attached to or referenced in the conversation")
    )
    story.append(
        bullet(
            "<b>Project Compass</b> — shows active Work metadata and a summary of the most relevant knowledge items"
        )
    )
    story.append(h3("Linking a Conversation to a Work"))
    story.append(
        bullet("Open a conversation → click the Work Badge selector at the top → choose a Work")
    )
    story.append(
        bullet(
    ...
    story.append(
        bullet("Toggle the brain icon to switch from the Workhorse model to the Reasoner model")
    )
    ...
    story.append(
        bullet(
            "<b>Header</b> — Upload button, search bar, filter chips (by readiness, Work, file type)"
        )
    )
    story.append(
        bullet(
            "<b>Document Cards / Table</b> — shows filename, type icon, readiness badge, knowledge count, linked Work, and upload date"
        )
    )
    story.append(
        bullet("<b>Work Filter Chip</b> — click to filter to only documents in a specific Work")
    )
    story.append(h3("Supported File Types"))
    story.append(
        bullet(
            "<b>PDF</b> — 3-tier extraction: pdfplumber (text) → pdf2image+Tesseract (OCR) → vision model"
        )
    )
    ...
    story.append(
        bullet("<b>MP3 / WAV / M4A / FLAC</b> — speech-to-text via Whisper / faster-whisper")
    )
    ...
    story.append(
        bullet(
            "Click <b>Upload</b> → drag files or click to browse → optionally assign to a Work before uploading"
        )
    )
    story.append(
        bullet(
    ...
    story.append(
        bullet(
            "<b>Document Header</b> — title, readiness badge, Reprocess button, Download button, Read Aloud button"
        )
    )
    story.append(
        bullet(
            "<b>Extraction Status</b> — warning messages from the pipeline if extraction had any issues"
        )
    )
    story.append(
        bullet(
            "<b>Overview Tab</b> — metadata (file type, size, sha256, upload date, linked Work selector, lifecycle state)"
        )
    )
    story.append(
        bullet(
            "<b>Knowledge Tab</b> — all AI-extracted and rule-based knowledge items with approve/reject review controls"
        )
    )
    story.append(
        bullet(
            "<b>Chunks Tab</b> — raw text chunks used for search and retrieval (with their embeddings status)"
        )
    )
    story.append(
        bullet(
            "<b>Chapters Tab</b> — for books processed with chapter detection: individual chapter records"
        )
    )
    story.append(
        bullet("<b>Read Aloud Panel</b> — text-to-speech playback of the document's extracted text")
    )
    story.append(
        bullet(
            "<b>Version Snapshots</b> — if multiple versions of this document exist, navigate between them"
        )
    )
    story.append(
        bullet(
            "<b>Related Documents</b> — near-duplicate detection results and version suggestions"
        )
    )
    story.append(h3("Reviewing Knowledge Items"))
    story.append(
        bullet("AI-extracted items (from the LLM) show a purple Sparkles badge — review carefully")
    )
    story.append(
        bullet("Rule-based items (from regex/pattern matching) are reliable and rarely need review")
    )
    ...
    story.append(
        bullet(
            "<b>Project Cards</b> — title, description, Work count, document count, concept count"
        )
    )
    ...
    story.append(
        bullet(
            "<b>Summary Cards</b> — Works in project, total documents, concepts, mastery progress"
        )
    )
    story.append(
        bullet(
            "<b>Concepts List</b> — learning concepts associated with this project with mastery labels"
        )
    )
    story.append(sp(8))
    ...
    story.append(
        bullet(
            "<b>Active Books</b> — Works with an active book pipeline in progress, showing stage, chapter count, and progress"
        )
    )
    story.append(
        bullet(
            "<b>In Progress</b> — Works that have started the book pipeline but are at an early stage"
        )
    )
    story.append(
        bullet("<b>Go to Works CTA</b> — when no books exist yet, prompts you to create a Work")
    )
    story.append(sp(8))
    ...
    story.append(
        bullet(
            "<b>Pre-flight Card</b> — automated checklist that must pass before export (all chapters present, no missing content, health score above threshold)"
        )
    )
    story.append(
        bullet("<b>Book Metadata Card</b> — title, subtitle, author, ISBN, publication date")
    )
    story.append(
        bullet(
            "<b>Chapters Card</b> — final chapter ordering table with drag-to-reorder and word count per chapter"
        )
    )
    story.append(
        bullet(
            "<b>Export Card</b> — generate DOCX or PDF export packages for test readers or publishers; recipient management and sign-off tracking"
        )
    )
    story.append(
        bullet(
            "<b>Product Spec</b> — physical or digital product specifications (trim size, paper type, format)"
        )
    )
    ...
    story.append(
        bullet(
            "<b>Brand Tokens</b> — typography, colour palette, and design tokens for the book's visual identity"
        )
    )
    story.append(sp(8))
    ...
    story.append(
        bullet(
            "<b>Topic / Course Controls</b> — filter by Work, concept, or search for a specific topic"
        )
    )
    story.append(
        bullet(
            "<b>Prerequisite Cards</b> — learning concepts you should understand before tackling others, shown with mastery level"
        )
    )
    story.append(
        bullet(
            "<b>Study Panel</b> — current question or concept with Socratic follow-ups; session limited to 5 questions by default"
        )
    )
    story.append(
        bullet(
            "<b>Progress Cards</b> — mastery bars per concept (novice → developing → proficient → mastery)"
        )
    )
    story.append(h3("How Socratic Learning Works"))
    story.append(
        bullet("The system asks you an open-ended question about a concept in your library")
    )
    story.append(
        bullet("Your answer is evaluated by the AI against known evidence from your knowledge base")
    )
    ...
    story.append(
        bullet("<b>Source Selection</b> — file upload area specifically for the intake batch")
    )
    story.append(bullet("<b>Metadata Fields</b> — tags, author, publication date, subject area"))
    story.append(
        bullet(
            "<b>Extraction Options</b> — choose extraction depth, enable/disable AI harvest, select relevant knowledge domains"
        )
    )
    story.append(
        bullet(
            "<b>Submit Area</b> — submit the intake for processing; status shows extraction progress"
        )
    )
    story.append(sp(8))
    ...
    story.append(
        bullet(
            "<b>Topic Cards</b> — each cluster of related knowledge items is grouped into a topic card with a name and summary"
        )
    )
    story.append(
        bullet(
            "<b>Related Connections</b> — clicking a topic shows which Works, documents, and other topics it connects to"
        )
    )
    story.append(sp(8))
    ...
    story.append(
        bullet(
            "<b>Graph Canvas</b> — force-directed node-link diagram; drag nodes to rearrange, scroll to zoom"
        )
    )
    story.append(
        bullet(
            "<b>Entity Kind Filter Chips</b> — show/hide People, Places, Organisations, Events, Concepts by type"
        )
    )
    ...
    story.append(
        bullet(
            "<b>Selection Panel</b> — click a node to see all knowledge items connected to that entity"
        )
    )
    story.append(h3("How to Use"))
    ...
    story.append(
        bullet(
            "<b>Action Selector</b> — dropdown of available actions (summarise, gap analysis, generate outline, etc.)"
        )
    )
    story.append(bullet("<b>Work Selector</b> — the Work the action should run against"))
    story.append(
        bullet("<b>Dynamic Input Form</b> — each action has its own schema of required inputs")
    )
    ...
    story.append(
        bullet(
            "<b>Overview / Status Header</b> — total LLM calls, average latency, error rate, most recent benchmark result"
        )
    )
    story.append(
        bullet(
            "<b>LLM Call Log</b> — searchable table of every AI call: timestamp, purpose, model, tokens in/out, latency, success/failure"
        )
    )
    story.append(
        bullet(
            "<b>Model Health Cards</b> — per-model health summary: uptime, p50/p95 latency, token throughput"
        )
    )
    story.append(
        bullet(
            "<b>Benchmark Results</b> — nightly quality tests with pass/fail and regression indicators"
        )
    )
    story.append(
        bullet(
            "<b>Prompt Registry</b> — view and health-check system prompts; prompts that regressed are flagged"
        )
    )
    story.append(
        bullet("<b>Policy / Settings</b> — enable/disable MCOS features, set benchmark schedule")
    )
    story.append(sp(8))
    ...
    story.append(
        bullet(
            "<b>Review Queue Tabs</b> — Knowledge Items, Document Findings, Benchmark Regressions — each with a count badge"
        )
    )
    story.append(
        bullet(
            "<b>Unverified Items List</b> — AI-extracted claims waiting for review, sorted by confidence (lowest first by default)"
        )
    )
    story.append(
        bullet(
            "<b>Item Detail</b> — full text, source document, extraction method, confidence score, and provenance chain"
        )
    )
    story.append(
        bullet(
            "<b>Confidence Explanation</b> — hover any confidence score to see what drove it up or down"
        )
    )
    story.append(
        bullet(
            "<b>Approve / Reject Controls</b> — accept a claim into the knowledge base, or reject it with an optional note"
        )
    )
    ...
    story.append(
        bullet("Approve high-confidence items in batch; review low-confidence items carefully")
    )
    story.append(
        bullet("Rejected items are soft-deleted — they remain in the database for audit purposes")
    )
    story.append(sp(8))
    ...
    story.append(
        bullet(
            "<b>Filter Controls</b> — filter by knowledge type, confidence band, or source document"
        )
    )
    story.append(
        bullet(
            "<b>Pending Knowledge Cards</b> — one card per item showing claim, source, confidence, and Accept/Reject buttons"
        )
    )
    story.append(
        bullet(
            "<b>Item Detail</b> — expand a card to see full provenance including which chunk of which document generated the claim"
        )
    )
    story.append(sp(8))
    ...
    story.append(
        bullet(
            "<b>LLM Status</b> — live connectivity check against your LLM server; shows model name and latency"
        )
    )
    story.append(
        bullet(
            "<b>Semantic Search / Embeddings</b> — embedding model health, circuit breaker state, Probe and Reindex buttons"
        )
    )
    story.append(
        bullet(
            "<b>Diagnostics</b> — run the full diagnostic suite: schema, integrity, orphans, connectivity, OCR/FFmpeg, data quality — results in categorised OK/WARN/ERROR format"
        )
    )
    story.append(
        bullet("<b>Nightshift</b> — last run time and result, Run Now button, next scheduled time")
    )
    story.append(
        bullet(
            "<b>Audio Enhancement</b> — enable/disable DeepFilterNet3 noise reduction for audio files"
        )
    )
    story.append(
        bullet(
            "<b>Settings</b> — enable/disable AI extraction, nightshift, deep-mode defaults, and other system-wide toggles"
        )
    )
    story.append(
        bullet(
            "<b>API Key</b> — view or regenerate the bearer token used by external API clients and scripts"
        )
    )
    ...
    story.append(
        bullet("When documents stop processing: check Diagnostics for 'stuck documents' warning")
    )
    ...
    story.append(
        bullet(
            "<b>Plan Panel</b> — describe what you want to generate; the AI creates a structured writing plan"
        )
    )
    story.append(
        bullet(
            "<b>Execute Panel</b> — approve the plan and run generation; streams the document in real time"
        )
    )
    story.append(
        bullet(
            "<b>Critique Loop</b> — the AI critiques its own output against your knowledge base, then revises"
        )
    )
    story.append(
        bullet(
            "<b>Output Panel</b> — final document with export to DOCX, Markdown, or direct library import"
        )
    )
    ...
    story.append(
        bullet("System → Nightshift → Run Now — verify the report shows no critical errors")
    )
    ...
    story.append(
        bullet("If the database is over 500 MB: System → Diagnostics → Run with --vacuum flag")
    )
    story.append(sp(6))
    ...
    story.append(
        bullet("Knowledge items tagged as 'rejected' are excluded from search and chat context")
    )
    story.append(sp(6))
    ...
    story.append(
        bullet(
            "<b>BIBLICAL</b> — auto-detected from scripture/theology keywords; searches biblegateway, biblehub, blueletterbible, ccel, thegospelcoalition, and 17 other curated sources"
        )
    )
    story.append(
        bullet(
            "<b>NEWS</b> — auto-detected from words like 'today', 'latest', '2026'; uses Tavily news lane"
        )
    )
    story.append(
        bullet(
            "<b>YOUTUBE</b> — searches YouTube and pulls full video transcripts (not just titles)"
        )
    )
    story.append(bullet("<b>FACEBOOK</b> — searches publicly-indexed Facebook pages only"))
    story.append(
        bullet(
            "<b>ACADEMIC</b> — restricts to arxiv, PubMed, JSTOR, Semantic Scholar, and similar scholarly sources"
        )
    )
    ...
    api_endpoints = [
        ...
        [
            "/api/library/upload",
            "POST",
            "Upload a file (multipart/form-data). Fields: file, work_id (optional)",
        ],
        ["/api/library/reprocess-all", "POST", "Bulk reprocess all stuck/error/no_text documents"],
        [
            "/api/library/{doc_id}/reprocess",
            "POST",
            "Reprocess a single document. Add ?force=true to force even if ready",
        ],
        [
            "/api/system/nightshift/run-now",
            "POST",
            "Trigger a full nightshift maintenance run immediately",
        ],
        ["/api/system/nightshift/status", "GET", "Last run time, result, and next scheduled time"],
        [
            "/api/system/embeddings/status",
            "GET",
            "Embedding model health, dimension, vector counts",
        ],
        [
            "/api/system/embeddings/probe",
            "POST",
            "Live-test the embedder and reset circuit breaker on success",
        ],
        [
            "/api/system/diagnostics",
            "GET",
            "Full system health report (schema, integrity, tools, LLM)",
        ],
        ["/api/works", "GET", "List all Works. Query: ?work_type=&status="],
        [
            "/api/works/{id}/stats",
            "GET",
            "Document count, knowledge count, conversation count for a Work",
        ],
        ...
        [
            [
                Paragraph(
                    ...
                    S["table_header"]
                    if i == 0
                    else (S["table_cell_code"] if j == 0 else S["table_cell"]),
                )
                for j, c in enumerate(row)
            ]
            for i, row in enumerate(api_endpoints)
        ],
    ...
    shortcuts = [
        ...
        [
            "Ctrl + C (on AI message)",
            "Copy message to clipboard (click copy button on the message)",
        ],
    ]
    story.append(
        two_col_table(["Shortcut", "Action"], shortcuts[1:], col_widths=[2.0 * inch, 4.5 * inch])
    )
    story.append(sp(10))
    ...
    terms = [
        ...
        (
            "Circuit Breaker",
            "A fault-tolerance pattern that stops retrying a failing service after N failures and retries automatically after a cooldown",
        ),
        (
            "Embeddings",
            "Dense numerical vectors representing text meaning, used for semantic similarity search",
        ),
        ...
        (
            "MCOS",
            "Model Calibration &amp; Observability System — Orivellum's LLM telemetry and quality framework",
        ),
        ("Nightshift", "The nightly maintenance daemon that keeps the database healthy"),
        (
            "PKLOS",
            "Policy Knowledge and Lifecycle Operating System — Orivellum's governance and audit framework",
        ),
        (
            "RAG",
            "Retrieval-Augmented Generation — injecting retrieved knowledge items as context before asking the LLM to answer",
        ),
        (
            "RRF",
            "Reciprocal Rank Fusion — combines ranked lists from multiple searches into one fused ranking",
        ),
        (
            "SSE",
            "Server-Sent Events — how the streaming chat responses are delivered to the browser",
        ),
        ("TTS", "Text-to-Speech — converts document text to audio using the configured TTS model"),
        (
            "WAL",
            "Write-Ahead Logging — SQLite mode that allows concurrent reads while a write is in progress",
        ),
        (
            "Work",
            "The top-level project container that groups documents, knowledge, conversations, and tasks",
        ),
    ]
    ...
    story.append(PageBreak())
    story.append(sp(100))
    story.append(HRFlowable(width="100%", thickness=1, color=GOLD, spaceAfter=16, spaceBefore=0))
    ...
    story.append(
        p("Re-run scripts/generate_training_manual.py to get a current version.", "title_page_ver")
    )
