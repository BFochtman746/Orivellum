...
if (Test-CommandExists uv) {
  ...
  # uv installs to %USERPROFILE%\.local\bin — add it to PATH
...
if ((Test-CommandExists tesseract) -or (Test-Path $tessPath)) {
  ...
  # UB-Mannheim build — most recent 5.x release
...
if (Test-CommandExists pdfinfo) {
  ...
  # Latest release from oschwartz10612/poppler-windows
  ...
  # The zip contains a single versioned folder; find and add its bin/
...
Write-Host "       pip install lemonade-server   (recommended)" -ForegroundColor Gray
Write-Host "       — OR —" -ForegroundColor Gray
Write-Host "       Download Ollama from https://ollama.com/download" -ForegroundColor Gray
Write-Host ""
Write-Host "    2. Start Orivellum:" -ForegroundColor White
